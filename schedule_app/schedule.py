...
form = h.get_form(['number','quarter','newquarter']+common.period_array())
number = form['number']
quarter = form['quarter']
newquarter = form['newquarter']
newSchedule = {key:form[key] for key in common.period_array()}

if not quarter:
    quarter = common.now_quarter()
user = operation_db.select_user(int(form['number']))
if newSchedule['Mon1'] != None:
    operation_db.save_sch(number,newquarter,newSchedule)
a,schedule = operation_db.select_sch(int(form['number']),quarter)
